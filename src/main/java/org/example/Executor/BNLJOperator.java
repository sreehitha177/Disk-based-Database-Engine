...
//    public Row next() {
//        while (true) {
//            // 1. Check if we still have matching rows for current inner row
//            if (matchingOuterRows != null && matchingOuterRows.hasNext()) {
//                Row outer = matchingOuterRows.next();
//                return new JoinedRow(outer, currentInnerRow);
//            }
//
//            // 2. Advance inner row
//            currentInnerRow = rightChild.next();
//
//            if (currentInnerRow != null) {
//                String innerKey = extractJoinKey(currentInnerRow);
//                List<Row> matches = hashTable.getOrDefault(innerKey, Collections.emptyList());
//                matchingOuterRows = matches.iterator();
//                // Try to return one immediately if possible
//                if (matchingOuterRows.hasNext()) {
//                    Row outer = matchingOuterRows.next();
//                    return new JoinedRow(outer, currentInnerRow);
//                }
//                // else go to next inner row
//            } else {
//                // 3. Exhausted current rightChild, go to next block
//                if (endOfLeft) return null;
//
//                loadNextOuterBlock();
//                rightChild.close();
...
//
//        int pagesUsed = 0;
//        while (pagesUsed < blockPageLimit) {
//            Row row = leftChild.next();
//            if (row == null) {
//                endOfLeft = true;
//                break;
...
//            return extractJoinKey(inner);
//        }
//        throw new IllegalArgumentException("Unsupported row type: " + row.getClass().getSimpleName());
//    }
//}
